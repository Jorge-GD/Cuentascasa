...
interface MensualPageProps {
  params: Promise<{
    cuentaId: string
    mes: string // Format: "2023-12"
  }>
}

export default function MensualPage({ params }: MensualPageProps) {
  ...
  const [fechaMes, setFechaMes] = useState<Date | null>(null)
  ...
  useEffect(() => {
    if (resolvedParams) {
      ...
      // Parsear el mes
      try {
        const fecha = parse(resolvedParams.mes, 'yyyy-MM', new Date())
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500">
              No se pudieron cargar los datos de la cuenta o el período solicitado.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Vista Mensual</h1>
          <p className="text-muted-foreground">
            {cuenta.nombre} - {format(fechaMes, 'MMMM yyyy', { locale: es })}
          </p>
        </div>
      </div>
